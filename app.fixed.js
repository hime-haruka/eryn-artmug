...
async function initCollab() {
  ...
  const items = normalizeCollabItems(rows).filter((it) => it.url);

  renderCollab(items);
  applyFxToAllWraps();
}

function normalizeCollabItems(rows) {
  return (rows || [])
    .filter((r) => {
      const a = (r.active ?? "").toString().trim();
      return a === "" || truthy(a);
    ...
    .map((r) => ({
      ...
      rawDiscount: (r.discount ?? "").toString().trim(),
    }))
    .filter((it) => it.name && !isPlaceholderCollabName(it.name))
    .sort((a, b) => (a.order || 999999) - (b.order || 999999) || a.name.localeCompare(b.name));
}

function isPlaceholderCollabName(name = "") {
  const t = name.trim().replace(/\s+/g, "");
  return !t || /추가예정/i.test(t);
}

function getSelectedCollabItem() {
  const sel = document.querySelector('[data-fm="collabArtist"]');
  const name = (sel?.value || "").trim();
  if (!name) return null;
  return APP_STATE.collabItems.find((it) => it.name === name) || null;
}

function getActiveEventDiscounts(rows) {
  return (rows || [])
    .filter((r) => truthy(r.active))
    .map((r) => {
      const amount = toInt(r.discount);
      if (!amount) return null;
      const label = (r.discountlabel || r.title || r.badge || r.subtitle || "이벤트 할인").toString().trim();
      return {
        key: `event:${label}`,
        type: "event",
        label,
...
function renderDiscountOptions(items) {
  const slot = document.querySelector('[data-fm="discountSlot"]');
  if (!slot) return;

  const discounts = (items || []).filter((it) => toInt(it.amount) > 0);
  ...
  slot.innerHTML = `
    <section class="fm-block fm-block--discount">
      <h3 class="fm-block-title">할인 적용</h3>
      <div class="fm-check-grid">
        ${discounts.map((it, idx) => {
          const id = `est-discount-${idx}`;
          return `
            <label class="fm-opt fm-opt--discount" for="${id}">
              <input
                id="${id}"
                type="checkbox"
                data-est="discount"
                data-discount-type="${escapeHtml(it.type || "event")}"
                data-title="${escapeHtml(it.label)}"
                data-price="${toInt(it.amount)}"
              >
              <span class="fm-opt-name">${escapeHtml(it.label)}</span>
              <span class="fm-opt-price">- ${escapeHtml(formatWon(it.amount))}</span>
            </label>
          `;
        }).join("")}
      </div>
...
function hasCollabEnabled() {
  const field = formValueByName("collab");
  return isAffirmativeValue(field);
}

function isAffirmativeValue(v) {
  const s = (v ?? "").toString().trim().toLowerCase();
  if (!s) return false;
  return /^(y|yes|true|1|있음|있다|유|예|네|진행|협업)$/i.test(s) || s.includes("있");
}

function formValueByName(name) {
...
function applyFxToAllWraps() {
  const targets = document.querySelectorAll(
    ".hero-wrap, .sc-wrap, .sp-wrap, .collab-wrap, .pr-wrap, .nt-wrap, .fm-wrap"
  );

  targets.forEach((wrap) => applyFxToWrap(wrap));
}

function applyFxToWrap(wrap) {
  if (!wrap) return;

  wrap.classList.add("fx-wrap");
  wrap.querySelectorAll(":scope > .fx-layer").forEach((n) => n.remove());
  ...
  const w = rect.width || wrap.offsetWidth || 1200;
  const h = rect.height || wrap.offsetHeight || 700;
  ...
  const isInSafeZone = (x, y) =>
    x >= safe.x1 && x <= safe.x2 && y >= safe.y1 && y <= safe.y2;
  ...
  function edgePoint() {
    ...
    if (side === 0) return { x: rand(0, w), y: rand(0, h * 0.10) };
    if (side === 1) return { x: rand(w * 0.90, w), y: rand(0, h) };
    if (side === 2) return { x: rand(0, w), y: rand(h * 0.90, h) };
    return { x: rand(0, w * 0.10), y: rand(0, h) };
  }

  function cornerNebula(corner) {
    const n = document.createElement("i");
    n.className = "fx-nebula";

    const nw = rand(180, 320);
    const nh = rand(120, 220);

    let x = 0, y = 0;
    if (corner === "tl") { x = rand(-60, 40); y = rand(-40, 30); }
    if (corner === "tr") { x = rand(w - 140, w - 20); y = rand(-40, 30); }
    if (corner === "bl") { x = rand(-60, 40); y = rand(h - 140, h - 20); }
    if (corner === "br") { x = rand(w - 140, w - 20); y = rand(h - 140, h - 20); }
  ...
  const nebulaCount = 17;

  for (let i = 0; i < nebulaCount; i++) {
    const corner = ["tl", "tr", "bl", "br"][Math.floor(Math.random() * 4)];
    cornerNebula(corner);
  }
  ...
  for (let i = 0; i < starCount; i++) {
    ...
    const size = Math.random() < 0.7 ? rand(2.2, 3.4) : rand(3.6, 5.4);
    if (size < 3.5) s.classList.add("tiny");
...
async function initForm() {
      ...
      <div class="content-wrap">
        ...
        <section class="fm-panel">
          ...
          <form class="fm-all" data-fm="form" autocomplete="off">
            ...
            <div class="fm-discount-slot" data-fm="discountSlot"></div>
  ...
  const [formRows, priceRows, eventRows, collabRows] = await Promise.all([
    fetchSheetRows(GIDS.form),
    fetchSheetRows(GIDS.price),
    fetchSheetRows(GIDS.event),
    fetchSheetRows(GIDS.collab),
  ]);

  APP_STATE.collabItems = normalizeCollabItems(collabRows);
...
function renderCollabAssistField(items) {
  const top = document.querySelector('[data-fm="top"]');
  if (!top) return;

  if (!document.querySelector('[data-fm="collabSelectSlot"]')) {
    top.insertAdjacentHTML("beforeend", `<div class="fm-item fm-collab-slot is-hidden" data-fm="collabSelectSlot"></div>`);
  }

  const slot = document.querySelector('[data-fm="collabSelectSlot"]');
  if (!slot) return;
  ...
  slot.innerHTML = `
    <label class="fm-label" for="fm-collab-artist">협업 작가 선택</label>
    <select class="fm-control" id="fm-collab-artist" data-fm="collabArtist" name="collab_artist" disabled>
      <option value="">협업 작가를 선택해주세요</option>
      ${items.map((it) => {
        const meta = it.discount > 0 ? ` · 리깅 ${formatWon(it.discount)} 할인` : "";
        return `<option value="${escapeHtml(it.name)}">${escapeHtml(withNim(it.name))}${escapeHtml(meta)}</option>`;
      }).join("")}
    </select>
    <p class="fm-help" data-fm="collabHelp">협업 진행 시에만 선택해주세요.</p>
  `;

  syncCollabFieldState();
}
...
function wireEstimateSystem() {
  ...
  const recalc = () => {
    ...
    discounts.forEach((item) => {
      total -= item.amount;
      lines.push(`${item.label} (-${formatWon(item.amount)})`);
    });
    ...
    if (breakdown) {
      breakdown.innerHTML = lines.length
        ? lines.map((line) => `<div class="fm-line">${escapeHtml(line)}</div>`).join("")
        : `<div class="fm-line muted">선택된 옵션이 없습니다.</div>`;
  ...
  form.addEventListener("change", (e) => {
    ...
    if (
      ...
      t.matches('input[data-est="discount"]') ||
      t.matches('select[data-est="unit"]') ||
      t.matches('[name="collab"]') ||
      t.matches('[name="collab_artist"]')
    ) recalc();
  });
  ...
  function getAppliedDiscounts() {
    ...
    const collabItem = getSelectedCollabItem();
    if (hasCollabEnabled() && collabItem?.discount > 0) {
      out.push({
        key: `collab:${collabItem.name}`,
        type: "collab",
        label: `${withNim(collabItem.name)} 협업 할인`,
        amount: collabItem.discount,
      });
    }

    document.querySelectorAll('input[data-est="discount"]:checked').forEach((cb) => {
      const amount = toInt(cb.dataset.price);
      if (!amount) return;
      out.push({
        key: `event:${cb.dataset.title}`,
        type: "event",
        label: cb.dataset.title || "이벤트 할인",
        amount,
      });
    });
  ...
  function buildCopyText() {
    ...
    for (const [k, v] of fd.entries()) {
      ...
      if (k === "collab_artist" && !hasCollabEnabled()) continue;
    ...
    const base = document.querySelector('input[name="est_base"]:checked');
    const extraAdds = [...document.querySelectorAll('input[type="checkbox"][data-est="add"]:checked')]
      .map((cb) => cb.dataset.title);
    const extraUnits = [...document.querySelectorAll('select[data-est="unit"]')]
      .map((sel) => {
        const n = toInt(sel.value);
        return n ? `${sel.dataset.title} x${n}` : "";
      })
      .filter(Boolean);
    const discounts = getAppliedDiscounts();
    ...
    rows.push("");
    rows.push("🧷 선택 리깅 옵션");
    rows.push(`- 기본 리깅 옵션: ${base ? base.dataset.title : "미선택"}`);

    rows.push("");
    rows.push("➕ 추가 옵션");
    if (extraAdds.length || extraUnits.length) {
      [...extraAdds, ...extraUnits].forEach((item) => rows.push(`- ${item}`));
    } else {
      rows.push("- 없음");
    }
    ...
    rows.push("💸 적용 할인");
    if (discounts.length) {
      discounts.forEach((item) => rows.push(`- ${item.label}: -${formatWon(item.amount)}`));
    } else {
      rows.push("- 없음");
    }
...
function syncCollabFieldState() {
  const slot = document.querySelector('[data-fm="collabSelectSlot"]');
  const select = document.querySelector('[data-fm="collabArtist"]');
  const help = document.querySelector('[data-fm="collabHelp"]');
  if (!slot || !select) return;

  const enabled = hasCollabEnabled();
  slot.classList.toggle("is-hidden", !enabled);
  ...
  if (help) {
    const item = getSelectedCollabItem();
    help.textContent = enabled
      ? item?.discount > 0
        ? `${withNim(item.name)} 선택 시 리깅 ${formatWon(item.discount)} 자동 할인`
        : "협업 작가를 선택해주세요."
      : "협업 진행 시에만 선택해주세요.";
  }
}
